...
fun categoryName(index: Int): String =
    CATEGORIES.getOrNull(index)?.name ?: "Unknown"
...
fun buildReportMarkdown(audit: Audit, findings: List<Finding>): String {
    ...
    sorted.forEachIndexed { i, f ->
        sb.appendLine(
            "| ${i + 1} | ${cell(f.title)} | ${cell(categoryName(f.categoryIndex))} | " +
